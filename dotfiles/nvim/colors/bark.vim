" bark - a warmer colorscheme, https://github.com/if-not-nil/bark
"
" Vendored: upstream ships a bare 'bark.vim' at the repo root rather than a
" plugin with a colors/ dir, so there is nothing for vim.pack to fetch.
" Re-sync with (from this directory):
"   curl -sLo colors/bark.vim https://raw.githubusercontent.com/if-not-nil/bark/main/bark.vim
" then re-apply the two local changes below.
"
" Upstream revision: be14cfb2266eca1acddc495c200a0c6c1ac039b1
" Local changes: added 'set background=dark' (upstream sets none), and named
" the scheme 'bark' (upstream is a :highlight dump still calling itself
" 'mydump', so ':colorscheme bark' would leave g:colors_name wrong).
